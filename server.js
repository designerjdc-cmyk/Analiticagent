...
app.get("/auth/callback",async(req,res)=>{const{code,error,error_description,state}=req.query;if(error)return res.redirect(`/?error=${encodeURIComponent(error_description||error)}`);if(!code)return res.redirect("/?error=No+code");const od=oauthStates.get(state);if(!od)return res.redirect("/?error=Expired");oauthStates.delete(state);
  ...
  console.log(`✅ @${p.username} | ${p.account_type} | f:${p.followers_count} | m:${p.media_count}`);
  ...
  // Auto-snapshot on connect
  await supa.from("snapshots").upsert({account_id:(await supa.from("accounts").select("id").eq("user_id",od.userId).eq("instagram_account_id",aid).single()).data?.id,followers_count:p.followers_count||0,follows_count:p.follows_count||0,media_count:p.media_count||0,snapshot_date:new Date().toISOString().slice(0,10)},{onConflict:"account_id,snapshot_date"}).then(()=>console.log("Snapshot saved")).catch(e=>console.warn("Snapshot fail:",e.message));
...
// API
app.get("/api/accounts",auth,async(req,res)=>{const{data}=await supa.from("accounts").select("*").eq("user_id",req.user.id).order("connected_at");res.json((data||[]).map(({access_token,...r})=>({...r,token_valid:r.token_expires_at>Date.now()})))});
app.delete("/api/accounts/:id",auth,async(req,res)=>{await supa.from("accounts").delete().eq("id",req.params.id).eq("user_id",req.user.id);res.json({ok:true})});

app.get("/api/accounts/:id/profile",auth,async(req,res)=>{const a=await getAcc(req.user.id,req.params.id);if(!a)return res.status(404).json({error:"Not found"});
  try{const r=await axios.get(`${IG}/me`,{params:{fields:"user_id,username,name,account_type,profile_picture_url,followers_count,follows_count,media_count,biography",access_token:a.access_token}});
  ...
  // Auto-snapshot on profile refresh
  await supa.from("snapshots").upsert({account_id:a.id,followers_count:r.data.followers_count||0,follows_count:r.data.follows_count||0,media_count:r.data.media_count||0,snapshot_date:new Date().toISOString().slice(0,10)},{onConflict:"account_id,snapshot_date"}).catch(()=>{});
  res.json({...r.data,id:a.id})}catch(e){handleErr(e,res)}});

app.get("/api/accounts/:id/insights",auth,async(req,res)=>{const a=await getAcc(req.user.id,req.params.id);if(!a)return res.status(404).json({error:"Not found"});
  ...
  try{const r=await axios.get(`${IG}/me/insights`,{params:p});res.json(r.data)}catch(e){const ms=safe.split(","),d=[];for(const m of ms){try{const r2=await axios.get(`${IG}/me/insights`,{params:{...p,metric:m}});if(r2.data?.data)d.push(...r2.data.data)}catch{}}if(d.length)return res.json({data:d});handleErr(e,res)}});

app.get("/api/accounts/:id/media",auth,async(req,res)=>{const a=await getAcc(req.user.id,req.params.id);if(!a)return res.status(404).json({error:"Not found"});
  const r=await tryMedia(a,req.query.limit||50,["id,caption,media_type,media_url,thumbnail_url,permalink,timestamp,like_count,comments_count","id,caption,media_type,thumbnail_url,permalink,timestamp,like_count,comments_count","id,caption,media_type,permalink,timestamp,like_count,comments_count"]);res.json(r.data)});

app.get("/api/accounts/:id/media-detailed",auth,async(req,res)=>{const a=await getAcc(req.user.id,req.params.id);if(!a)return res.status(404).json({error:"Not found"});
  ...
  // Save snapshot with engagement data
  ...
  await supa.from("snapshots").upsert({account_id:a.id,followers_count:a.followers_count||0,follows_count:a.follows_count||0,media_count:a.media_count||0,avg_engagement:avgEng,avg_reach:avgReach,total_likes:totalLikes,total_comments:totalComments,snapshot_date:new Date().toISOString().slice(0,10)},{onConflict:"account_id,snapshot_date"}).catch(()=>{});
  console.log(`✅ ${enriched.length} posts enriched`);res.json({data:enriched,followers_count:a.followers_count||0,total_fetched:enriched.length})});

app.get("/api/accounts/:id/demographics",auth,async(req,res)=>{const a=await getAcc(req.user.id,req.params.id);if(!a)return res.status(404).json({error:"Not found"});
  try{const r=await axios.get(`${IG}/me/insights`,{params:{metric:"engaged_audience_demographics,reached_audience_demographics,follower_demographics",period:"lifetime",metric_type:"total_value",timeframe:"last_30_days",access_token:a.access_token}});res.json(r.data)}catch(e){handleErr(e,res)}});

app.post("/api/accounts/:id/refresh-token",auth,async(req,res)=>{const a=await getAcc(req.user.id,req.params.id);if(!a)return res.status(404).json({error:"Not found"});
  try{const r=await axios.get(`${IG}/refresh_access_token`,{params:{grant_type:"ig_refresh_token",access_token:a.access_token}});await supa.from("accounts").update({access_token:r.data.access_token,token_expires_at:Date.now()+r.data.expires_in*1000}).eq("id",a.id);res.json({ok:true})}catch(e){handleErr(e,res)}});

// Snapshots API
app.get("/api/accounts/:id/snapshots",auth,async(req,res)=>{const a=await getAcc(req.user.id,req.params.id);if(!a)return res.status(404).json({error:"Not found"});
  const limit=parseInt(req.query.limit)||90;
  const{data,error}=await supa.from("snapshots").select("*").eq("account_id",a.id).order("snapshot_date",{ascending:true}).limit(limit);
...
function handleErr(e,r){const x=e.response?.data?.error;r.status(e.response?.status||500).json({error:x?.message||e.message,type:x?.type,code:x?.code})}
